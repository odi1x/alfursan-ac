...
export default async function Home() {
  ...
  return (
    <>
      {/* ======================= Hero ======================= */}
      <header className="relative pt-[10vh] sm:pt-[12vh] pb-[32px] sm:pb-[40px] px-0 overflow-hidden min-h-[480px] sm:min-h-[580px] border-b border-line flex flex-col">
        <div
      ...
      <div className="border-b border-line relative z-10 bg-bg">
        <div className="w-[min(100%,1060px)] mx-auto px-[20px] py-[16px] sm:py-[22px] grid grid-cols-3 gap-[10px] divide-x divide-x-reverse divide-line text-center">
          {stats.map((s, i) => (
            <div key={i}>
      ...
      {/* ======================= اللاعبون المنتقلون ======================= */}
      <section id="players" className="pt-[36px] sm:pt-[54px] pb-[40px] sm:pb-[60px] overflow-hidden" aria-labelledby="players-title">
        <div className="w-[min(100%,1060px)] mx-auto px-[20px] mb-[16px] sm:mb-[26px]">
        ...
        <div className="flex gap-[12px] px-[20px] pb-[12px] w-max animate-infinite-scroll hover:animation-paused active:animation-paused">
          ...
          {[...players, ...players].map((p, i) => (
            <article key={i} className="flex-none basis-[136px] sm:basis-[190px] bg-surface border border-line rounded-[16px] overflow-hidden" aria-hidden={i >= players.length ? 'true' : undefined}>
              ...
              <div className="p-[10px] sm:p-[14px] pt-[2px] text-center">
                <div className="font-display font-semibold text-[.9rem] sm:text-[1rem] leading-[1.5] mt-2">{p.name}</div>
                <span className={`inline-block mt-[4px] sm:mt-[6px] text-[.7rem] sm:text-[.74rem] font-semibold px-[8px] py-[2px] sm:px-[12px] sm:py-[3px] rounded-full text-white ${getClubColorClass(p.club)}`}>{p.club}</span>
                <span className="block text-muted text-[.68rem] sm:text-[.72rem] mt-[4px] sm:mt-[5px]">مواليد {p.year}</span>
              </div>
            </article>
          ))}
        </div>
      ...
      {/* ======================= الفروع ======================= */}
      <section id="branches" className="bg-surface pt-[36px] sm:pt-[54px] pb-[40px] sm:pb-[60px]" aria-labelledby="branches-title">
        <div className="w-[min(100%,1060px)] mx-auto px-[20px]">
          ...
          <div className="grid gap-[12px] sm:gap-[16px] mt-[16px] sm:mt-[26px] sm:grid-cols-2" id="branch-grid">
            {branches.map((b) => (
              <Link key={b.id} className="relative block bg-surface-2 border border-line rounded-[16px] py-[16px] px-[16px] sm:py-[24px] sm:px-[22px] overflow-hidden transition-colors duration-200 active:border-academy-red before:content-[''] before:absolute before:inset-y-0 before:start-0 before:w-[6px] before:bg-gradient-to-b before:from-academy-red before:to-academy-red-deep" href={`/branch/${b.slug}`}>
                <div className="font-display font-bold text-[1.2rem] sm:text-[1.35rem]">{b.name}</div>
                <div className="hidden sm:block text-muted text-[.88rem] mt-[2px]">{b.landmark}</div>
                <div className="inline-flex items-center gap-[6px] mt-[6px] sm:mt-[12px] text-[.78rem] sm:text-[.82rem] text-[#FFD666] font-semibold">
                  ⭐ {b.rating} <span className="text-muted font-normal">({b.ratingCount} تقييم على Google)</span>
                </div>
                <span className="flex items-center gap-[8px] mt-[10px] sm:mt-[16px] font-display font-semibold text-academy-red text-[.9rem] sm:text-[.98rem] after:content-['⟵']">الأسعار والمواعيد والموقع</span>
              </Link>
            ))}
          </div>
        </div>
      ...
      {/* ======================= التقييمات ======================= */}
      <section id="reviews" className="pt-[36px] sm:pt-[54px] pb-[40px] sm:pb-[60px]" aria-labelledby="reviews-title">
        ...
        <div className="flex gap-[12px] sm:gap-[14px] overflow-x-auto px-[20px] pt-[16px] sm:pt-[24px] pb-[10px] snap-x snap-mandatory scrollbar-none">
          {reviews.map((r) => (
            <article key={r.id} className="flex-none basis-[min(82vw,330px)] bg-surface border border-line rounded-[16px] p-[16px] sm:p-[20px] flex flex-col gap-[8px] sm:gap-[10px]">
      ...
      {/* ======================= CTA ختامي ======================= */}
      <section className="py-[48px] sm:py-[80px] relative border-t border-line overflow-hidden">
        ...
        <div className="w-[min(100%,1060px)] mx-auto px-[20px] text-center relative z-10">
          <h2 className="text-[clamp(1.8rem,6vw,3rem)] font-display font-[900] mb-[12px] sm:mb-[20px]">جاهز لتكون البطل القادم؟</h2>
          <p className="text-[#DFE1E8] font-display text-[1rem] sm:text-[1.2rem] max-w-[600px] mx-auto mb-[24px] sm:mb-[40px]">انضم إلى مئات اللاعبين الذين بدأوا رحلتهم الاحترافية معنا. سجل الآن واحجز مكانك.</p>
          <a className="inline-flex items-center justify-center font-display font-bold text-[1.1rem] sm:text-[1.2rem] rounded-full px-[36px] sm:px-[48px] min-h-[56px] sm:min-h-[64px] bg-academy-red text-white shadow-[0_8px_32px_rgba(226,7,19,0.4)] transition-transform duration-200 active:scale-95" href="#branches">انضم للأكاديمية الآن</a>
        </div>
      </section>

      <footer className="border-t border-line pt-[24px] sm:pt-[36px] pb-[80px] sm:pb-[100px] text-center">
